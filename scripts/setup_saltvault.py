...
def prompt_ssl_mode() -> str:
    ...
    print("  4) tailscale (use 'tailscale cert <hostname>' to generate)")
    choice = input("Choice [1]: ").strip() or '1'
    mapping = {'1': 'self-signed', '2': 'existing', '3': 'cloudflare', '4': 'tailscale'}
...
def render_nginx(domain: str, ssl_mode: str):
    ...
    if ssl_mode == 'cloudflare':
        ...
        # Tailscale certificate mode (certs generated by 'tailscale cert')
        server {{
            listen 443 ssl;
            server_name {domain};
            ssl_certificate /etc/nginx/certs/fullchain.pem;
            ssl_certificate_key /etc/nginx/certs/privkey.pem;
            # Optional: restrict cipher suites if desired
            location / {{
                proxy_pass http://web;
...
def main():
    ...
    if ssl_mode == 'self-signed':
        ...
        header("Generate Tailscale Certificate")
        print("Ensure this host is already logged into Tailscale and domain resolves via MagicDNS.")
        generate_tailscale_cert(domain)
    else:
